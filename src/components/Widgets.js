...
export const DescriptionWidget2 = ({info, title}) => {
  console.log(info);
    ...
    <Card border="light" className="shadow-sm">
      <Card.Body>
        ...
        {/* {info.map((data, index) => (
          ...
          <p className="m-0"> <span className="h6 m-0">Price:</span> {data.price}</p>
          {data.categories.map((data, index) => (
            <p className="m-0" key={index}> <span className="h6 m-0">categories:</span> {data}</p>
          ))}
